...
def slice_search(user_query):
    ...
    letters = user_query
    slices = [letters[0:3], letters[1:4], letters[2:5], letters[3:6],
              letters[4:7]]
    ...
    for slce in slices:
        if (slce != '') and (len(slce) > 1):
            new_slices.append(slce)

    found = []

    for new in new_slices:
        for t in terms:
            if new in t['term_name']:
                found.append(t['term_name'])
    ...
    for term in found:
        first_letters = term[:3]
        if first_letters == slices[0]:
            first_results.add(term)
        else:
            remaining.add(term)

    results = list(first_results)+list(remaining)
...
def inject_user():
    ...
    try:
        ...
        g.access_level = mongo.db.users.find_one({
            "_id": ObjectId(user_id)})[
            'access_level'].lower() if user_id else 'requested'
...
def inject_notifications():
    g.new_registrations = list(mongo.db.users.find({
    ...
    g.notifications = len(g.suggested_terms) + len(
        g.flagged_comments) + len(g.new_registrations)
    g.term_updates = list(mongo.db.terms.find().sort(
        "last_updated", -1).limit(5))
    g.recent_comments = list(mongo.db.comments.find().sort(
        "_id", -1).limit(5))
    ...
    def manage_suggested_terms(suggestion):
        ...
        try:
            suggested_by = mongo.db.users.find_one({
                '_id': ObjectId(suggestion_user)})['username']
    ...
    def term_comment(comment):
        try:
            ...
            related_term = mongo.db.terms.find_one({
                '_id': ObjectId(rel_term_id)})
    ...
    return dict(categories=g.categories, term_comment=term_comment,
                manage_flagged_comments=manage_flagged_comments,
                manage_suggested_terms=manage_suggested_terms,
                notifications=g.notifications,
                new_registrations=g.new_registrations,
                suggested_terms=g.suggested_terms,
                flagged_comments=g.flagged_comments,
                term_updates=g.term_updates,
                recent_comments=g.recent_comments, levels=g.levels)
...
def get_pinned_terms():
    ...
    try:
        ...
        g.pinned_terms = list(mongo.db.terms.find({
            "_id": {"$in": pinned_term_ids}}))
...
def dashboard():
    if check_user_login() is True:
        ...
        return render_template("dashboard.html", terms=terms, levels=levels)
...
def get_category(category):
    if check_user_login() is True:
        terms = list(mongo.db.terms.find({
            'term_name': {"$regex": '^' + category, "$options": 'i'}}))
...
def view_term(term_id):
    if check_user_login() is True:
        ...
        def find_commenter(term_comment):
            ...
            try:
                term_comment_user = mongo.db.users.find_one({
                    '_id': ObjectId(term_comment['user'])})
...
def pin_term(term_id):
    if check_user_login() is True:
        ...
        user_record_field = mongo.db.users.count_documents({
            "$and":  [user_query, {"pinned_terms": ObjectId(term_id)}]})
...
def remove_pin(pinned_term):
    if check_user_login() is True:
        ...
        mongo.db.users.update_one({"username": username}, {
            "$pull": {"pinned_terms": ObjectId(pinned_term)}})
...
def delete_term(term_id):
    if check_user_login() is True:
        ...
        find_pins_users = list(mongo.db.users.find({
            "pinned_terms": {"$in": [ObjectId(term_id)]}}))
        # delete pin references from user accounts
        for pin_user in find_pins_users:
            mongo.db.users.update_one(pin_user, {
                "$pull": {"pinned_terms": ObjectId(term_id)}})
...
def manage_users():
    if check_user_login() is True:
        ...
        return render_template("manage_users.html",
                               users_list=users_list, levels=levels)
...
def show_all_users():
    if check_user_login() is True:
        levels = list(mongo.db.access_levels.find().sort("level_name", 1))
        users_list = list(mongo.db.users.find().sort([(
            "access_level", 1), ("username", 1)]))
...
def add_user():
    if check_user_login() is True:
        ...
        if request.method == "POST":
            ...
            users_list = list(mongo.db.users.find({"$text": {
                "$search": new_username}}))
            levels = list(mongo.db.access_levels.find().sort("level_name", 1))
